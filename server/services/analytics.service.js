...
import { milesToKilometers } from '../../src/utils/distance.js'
...
function getRouteOptimization(fuelAnalytics) {
  ...
  const scoredRoutes = initialRoutes.map((route) => {
    ...
    return {
      ...
      distanceKilometers: milesToKilometers(route.distanceMiles),
      ...
      estimatedCostPerKilometer: route.distanceMiles ? roundCurrency(estimatedFuelCost / milesToKilometers(route.distanceMiles)) : 0,
      efficiencyScore,
  ...
  return {
    ...
    recommendations: [
      ...
      },
      {
        ...
        status: 'Active',
        ...lowestFuelCost,
        reasons: ['Lowest estimated fuel cost from route distance, estimated MPG, and average fuel price.'],
      },
      {
        id: 'lowest-toll',
...
function buildAssistantCapabilities({ revenue, fuel, fleet, drivers, operations, financial, routeOptimization }) {
  return {
    routeOptimization: {
      summary: `${routeOptimization.scoredRoutes.length} routes scored using stored route distance in kilometers, estimated hours, and fuel assumptions.`,
